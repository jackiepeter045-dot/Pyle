...
export default function Login() {
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center max-w-sm mx-auto px-5 py-12">
      ...
      <p className="text-muted text-sm mb-7">Sign in to your Pyle account.</p>
      <form onSubmit={submit} className="flex flex-col gap-3">
        ...
        <input type="password" required placeholder="Password" value={password} onChange={e=>setPassword(e.target.value)} className="border border-line rounded-2xl px-4 py-3 text-sm outline-none focus:border-forest"/>
        {error && <p className="text-pink-deep text-sm bg-pink-tint rounded-xl px-3 py-2">{error}</p>}
        <button type="submit" disabled={busy} className="bg-forest text-white font-semibold py-3 rounded-full disabled:opacity-60 mt-1">{busy?"Signing in…":"Sign in"}</button>
      </form>
      ...
      <p className="text-sm text-muted mt-7 text-center">No account? <Link to="/signup" className="text-pink-deep font-semibold">Sign up</Link></p>
    </div>
  );
}
