          ...
          <div className="flex flex-col gap-3">
            {stores.map(s => (
              <div key={s.id} className="border border-line rounded-2xl p-4 flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-mint flex items-center justify-center text-lg shrink-0">{s.logo_url ? <img src={s.logo_url} className="w-full h-full rounded-full object-cover" alt={s.name}/> : "🏪"}</div>
